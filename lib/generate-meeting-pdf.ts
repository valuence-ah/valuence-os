...
export async function generateMeetingPDFBuffer(
  ...
  const dateStr = meeting.date
    ...
    : "";
  if (dateStr)    { doc.text(`Date: ${dateStr}`, margin, y); y += 5; }
  if (meeting.duration_minutes) { doc.text(`Duration: ${meeting.duration_minutes} minutes`, margin, y); y += 5; }
  ...
  body(fmt.overview ?? "Not available");
  ...
  // Fallback: if no structured content, show raw summary
  if (!fmt.hasStructure && fmt.rawFallback) {
    heading("SUMMARY");
  ...
  doc.setFont("helvetica", "italic");
  doc.setTextColor(...GREY);
  doc.text(`Generated by Valuence OS — ${today}`, margin, pageH - 12);
  doc.text("Confidential — Valuence Ventures", margin, pageH - 7);
