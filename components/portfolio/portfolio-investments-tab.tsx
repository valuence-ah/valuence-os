...
function boardLabel(v: string | null | undefined): string {
  if (!v) return "—";
  return BOARD_REP_OPTIONS.find(o => o.value === v)?.label ?? v;
}
...
// ── Generate a signed download URL and open it ────────────────────────────────
    ...
    <div className="flex-1 min-w-0">
      ...
      {file ? (
        <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
          ...
          <button type="button" onClick={() => onChange(null)} className="text-green-400 hover:text-red-500 flex-shrink-0 transition-colors">
            <X size={12} />
          </button>
        </div>
      ) : (
...
// ── 3-column investment form fields (shared by Add + Edit) ───────────────────
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <select value={form.board_representation} onChange={e => setForm(p => ({ ...p, board_representation: e.target.value as "board_seat" | "board_observer" | "no" }))}
            className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      {isSafe ? (
        ...
        <div className="grid grid-cols-3 gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
          ...
          <div>
            <label className="text-[10px] font-semibold text-blue-600 uppercase tracking-wide block mb-1">ESOP (%)</label>
            <input type="number" value={form.esop} onChange={e => setForm(p => ({ ...p, esop: e.target.value }))}
...
function InvestmentTile({
  ...
  const dataFields = [
    ...
    ...(isSafe
      ? [
          { label: "Val. Cap",  value: fmtMoney(inv.valuation_cap) },
          { label: "Discount",  value: fmtPct(inv.discount) },
          ...(inv.interest_rate !== null ? [{ label: "Interest", value: fmtPct(inv.interest_rate) }] : []),
        ]
      : [
          { label: "Pre-Money", value: fmtMoney(inv.pre_money_valuation) },
          { label: "Ownership", value: fmtPct(inv.ownership_pct) },
          ...(inv.esop !== null ? [{ label: "ESOP", value: fmtPct(inv.esop) }] : []),
        ]
    ),
  ];
  ...
  async function handleSave() {
    setSaving(true);
    const supabase = createClient();
    const isSafeForm = form.investment_type === "safe";
    const updates: Partial<PortfolioInvestment> = {
      ...
      valuation_cap:        isSafeForm && form.valuation_cap ? Number(form.valuation_cap) : null,
      discount:             isSafeForm && form.discount ? Number(form.discount) : null,
      interest_rate:        isSafeForm && form.interest_rate ? Number(form.interest_rate) : null,
      pre_money_valuation:  !isSafeForm && form.pre_money_valuation ? Number(form.pre_money_valuation) : null,
      ownership_pct:        !isSafeForm && form.ownership_pct ? Number(form.ownership_pct) : null,
      esop:                 !isSafeForm && form.esop ? Number(form.esop) : null,
  ...
  return (
    <div className={cn("bg-white border rounded-xl shadow-sm overflow-hidden transition-shadow", isSafe ? "border-violet-200" : "border-blue-200", isOpen && "shadow-md")}>
      ...
      {/* ── Single summary row — click to expand ── */}
      <div
      ...
      >
        {/* Cell 0: Round + type */}
        <div className="px-3 py-2.5 flex flex-col justify-center flex-shrink-0 min-w-[90px]">
        ...
        >
          ...
          {confirmDelete ? (
            ...
            <>
              ...
              <button onClick={e => { e.stopPropagation(); setConfirmDelete(true); }} className="p-0.5 text-slate-300 hover:text-red-500 transition-colors">
                <Trash2 size={12} />
              </button>
            </>
          )}
          <ChevronDown size={13} className={cn("text-slate-400 transition-transform duration-200 ml-1", isOpen && "rotate-180")} />
        </div>
      </div>
          ...
          {editing ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={cancelEdit} className="flex-1 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50">
                  Cancel
                </button>
            ...
            <div className="space-y-3">
              {/* Extra details not in the summary row */}
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-600">
                {inv.board_representation && inv.board_representation !== "no" && (
                  <span><span className="text-slate-400">Board: </span>{boardLabel(inv.board_representation)}</span>
                )}
              ...
              <div className="flex gap-3">
                <DocUpload
                  ...
                  existingPath={memoPath}
                  onUploaded={(path, name) => { setMemoPath(path); setMemoName(name); }}
                />
                <DocUpload
                  investmentId={inv.id}
                  ...
                  existingPath={subDocPath}
                  onUploaded={(path, name) => { setSubDocPath(path); setSubDocName(name); }}
                />
              </div>
            </div>
          )}
...
function AddInvestmentForm({
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      investment_type:      form.investment_type,
      board_representation: form.board_representation,
      valuation_cap:        isSafe && form.valuation_cap ? Number(form.valuation_cap) : null,
      discount:             isSafe && form.discount ? Number(form.discount) : null,
      interest_rate:        isSafe && form.interest_rate ? Number(form.interest_rate) : null,
      pre_money_valuation:  !isSafe && form.pre_money_valuation ? Number(form.pre_money_valuation) : null,
      ownership_pct:        !isSafe && form.ownership_pct ? Number(form.ownership_pct) : null,
      esop:                 !isSafe && form.esop ? Number(form.esop) : null,
      notes:                form.notes || null,
    };
    ...
    if (memoFile) {
      ...
      const { error: upErr } = await supabase.storage.from("investment-memos").upload(path, memoFile, { upsert: true });
      if (!upErr) {
        await supabase.from("portfolio_investments").update({ memo_storage_path: path, memo_file_name: memoFile.name, updated_at: new Date().toISOString() }).eq("id", id);
    ...
    if (subDocFile) {
      ...
      const { error: upErr } = await supabase.storage.from("investment-memos").upload(path, subDocFile, { upsert: true });
      if (!upErr) {
        await supabase.from("portfolio_investments").update({ subscription_doc_storage_path: path, subscription_doc_file_name: subDocFile.name, updated_at: new Date().toISOString() }).eq("id", id);
