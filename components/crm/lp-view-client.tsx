...
import {
  ...
  BarChart2, AlertCircle, CheckSquare, Video, Clock,
  ChevronDown, MoreHorizontal, Loader2, ArrowUpRight, FileText,
  Pencil, Check, Building2, Settings,
} from "lucide-react";
...
type LpTypeOption = typeof LP_TYPE_OPTIONS[number];
...
function getLpTypeBadge(lpType: string | null): string {
  ...
  return LP_TYPE_BADGE[lpType] ?? "bg-gray-100 text-gray-600";
}
...
// DDQ status — read-only, derived from stage
function getDdqStatus(stage: string | null): { label: string; color: string } {
  if (!stage || stage === "Lead" || stage === "Initial Meeting")
...
// Tier — read-only, derived from priority
...
function CompanyLogo({ company, size = "md" }: { company: Company; size?: "sm" | "md" | "lg" }) {
  ...
  if (logoSrc && !imgError) {
    return (
      <img src={logoSrc} alt={company.name} onError={() => setImgError(true)}
...
function MetricCard({
  label, value, sub, icon: Icon, accent, editable, onEdit,
}: {
...
function PrepBriefModal({
  company, contacts, interactions, onClose,
}: {
  ...
  const prob = calcProbPct(company.lp_stage);
      ...
      >
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Stage", value: company.lp_stage ?? "Not set" },
              { label: "Commit Goal", value: formatCompact(company.commitment_goal) },
              { label: "Expected", value: expected != null ? formatCompact(expected) : "—" },
            ].map((item) => (
          ...
          <div>
            ...
            <div className="space-y-1.5 text-sm text-slate-700">
              ...
              {company.aum && <p><span className="text-slate-400">AUM:</span> {formatCompact(company.aum)}</p>}
              {(company.location_city || company.location_country) && (
                <p><span className="text-slate-400">Location:</span> {[company.location_city, company.location_country].filter(Boolean).join(", ")}</p>
              )}
            ...
            <div>
              ...
              <div className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl">
                ...
                <div>
                  ...
                  {(primary.location_city || primary.location_country) && (
                    <p className="text-xs text-slate-400">{[primary.location_city, primary.location_country].filter(Boolean).join(", ")}</p>
                  )}
                </div>
              </div>
            </div>
...
export function LpViewClient({ initialCompanies }: Props) {
  ...
  // ── Fund target (manually set, persisted in localStorage) ──────────────────
  ...
  function saveFundTarget() {
    const num = parseFloat(fundTargetInput.replace(/[^0-9.]/g, ""));
  ...
  useEffect(() => {
    if (!showUpdateStage) return;
    function handler(e: MouseEvent) {
      if (updateStageRef.current && !updateStageRef.current.contains(e.target as Node)) {
    ...
    async function load() {
      ...
      for (const row of data) {
        if (row.company_id && !map[row.company_id]) {
          map[row.company_id] = { date: row.date, type: row.type };
        }
      }
      setLastTouchMap(map);
    }
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("contacts").select("id, first_name, last_name, company_id").in("company_id", ids);
      if (!data) return;
  ...
  const metrics = useMemo(() => {
    ...
    const softCircled = companies.filter((c) => c.lp_stage === "Due Diligence" || c.lp_stage === "Discussion in Process");
    ...
    const goalsWithVal  = companies.filter((c) => c.commitment_goal != null);
    const avgCheck      = goalsWithVal.length > 0
      ? goalsWithVal.reduce((s, c) => s + (c.commitment_goal ?? 0), 0) / goalsWithVal.length : 0;
    const convRate      = companies.length > 0
      ? Math.round((committed.length / companies.length) * 100) : 0;

    return {
      committedAmt, softAmt, pipelineAmt, avgCheck, convRate,
  ...
  const filtered = useMemo(() => {
    ...
    return companies.filter((c) => {
      ...
      if (activeFilter === "overdue") {
        const last = lastTouchMap[c.id]?.date;
        if (!last) return true;
        return (Date.now() - new Date(last).getTime()) / 86_400_000 > 30;
      }
  ...
  async function handleAddActivity() {
    ...
    const subjectMap = { call: "Call", meeting: "Meeting", email: "Email" };
    const { data: newInt } = await supabase.from("interactions").insert({
      company_id: selected.id,
      type: activityType,
      date: activityDate,
      subject: subjectMap[activityType],
      body: activityNote.trim() || null,
      created_by: user?.id ?? null,
    }).select().single();
    setSavingActivity(false);
    if (newInt) {
      setInteractions((prev) => [newInt as Interaction, ...prev]);
      // Update last touch map
      setLastTouchMap((prev) => ({
        ...prev,
        [selected.id]: { date: activityDate, type: activityType },
      }));
    }
    setAddingActivity(false);
    setActivityNote("");
  ...
  async function handleAddContact() {
    ...
    const { data: newC } = await supabase.from("contacts").insert({
      ...
      location_city: newContact.location_city.trim() || null,
      location_country: newContact.location_country.trim() || null,
      company_id: selected.id,
      type: "lp" as const,
  ...
  const mandateScores = useMemo(() => {
    ...
    const stageScore = ({ "Lead": 20, "Initial Meeting": 35, "Discussion in Process": 55, "Due Diligence": 75, "Committed": 95, "Passed": 10 }[selected.lp_stage ?? ""] ?? 20);
    const ticketScore = selected.commitment_goal ? Math.min(100, Math.round((selected.commitment_goal / 5_000_000) * 100)) : 30;
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden bg-slate-50">
      ...
      <div className="px-5 py-4 border-b border-slate-200 bg-white">
        {/* Fund target edit popover */}
        {editingFundTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30"
            onClick={() => setEditingFundTarget(false)}>
            <div className="bg-white rounded-xl shadow-xl p-5 w-72" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-xs text-slate-500 mb-3">Enter the total fundraising goal for this fund.</p>
              <input
                ref={fundTargetRef}
                type="number"
        ...
        <div className="flex gap-3 mb-4">
          <MetricCard label="Fund Target"
            value={fundTarget > 0 ? formatCompact(fundTarget) : "Set target"}
            sub={`${companies.length} LPs`} icon={Target} accent="bg-blue-500"
            editable onEdit={() => setEditingFundTarget(true)}
          />
          <MetricCard label="Committed" value={formatCompact(metrics.committedAmt || null)}
            sub={fundTarget > 0 ? `${formatPct(metrics.committedPct / 100)} of target` : undefined}
            icon={CheckSquare} accent="bg-emerald-500"
          />
          <MetricCard label="Soft-circled" value={formatCompact(metrics.softAmt || null)}
            sub={fundTarget > 0 ? `${formatPct(metrics.softPct / 100)} of target` : undefined}
            icon={TrendingUp} accent="bg-amber-500"
          />
          <MetricCard label="Active Pipeline" value={String(metrics.activeCount)}
            sub={formatCompact(metrics.pipelineAmt || null)} icon={BarChart2} accent="bg-violet-500"
          />
          <MetricCard label="Avg Check Size" value={formatCompact(metrics.avgCheck || null)}
            icon={DollarSign} accent="bg-indigo-500"
          />
          <MetricCard label="Conversion Rate" value={`${metrics.convRate}%`}
        ...
        {fundTarget > 0 && (
          <div>
            ...
            <div className="flex gap-4 mt-1.5">
              {[
                { color: "bg-emerald-500", label: "Committed", pct: metrics.committedPct },
                { color: "bg-amber-400",   label: "Soft-circled", pct: metrics.softPct },
                { color: "bg-blue-300",    label: "Pipeline", pct: metrics.pipelinePct },
              ].map((item) => (
                <span key={item.label} className="flex items-center gap-1 text-[10px] text-slate-500">
                  ...
                  {item.label} {formatPct(item.pct / 100)}
                </span>
              ))}
            </div>
          </div>
          ...
          <table className="w-full text-sm border-collapse min-w-[1100px]">
            <thead className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                {["Company","LP Type","Tier","Stage","Commit Goal","Expected","Prob %","Last Touchpoint","Next Follow-up","DDQ Status","Strategic Value","City"].map((col) => (
                  <th key={col} className="text-left px-3 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap">
                    {col}
                  </th>
                ))}
                <th className="w-8 px-3 py-2.5" />
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                const daysAgo   = touch ? Math.floor((Date.now() - new Date(touch.date).getTime()) / 86_400_000) : null;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-600 whitespace-nowrap">{getTier(co.priority)}</td>
                    <td className="px-3 py-2.5">
                      {co.lp_stage ? (
                        <div className="flex items-center gap-1.5">
                          <span className={cn("w-2 h-2 rounded-full flex-shrink-0", STAGE_DOT[co.lp_stage] ?? "bg-slate-300")} />
                          <span className={cn("text-xs font-medium whitespace-nowrap", STAGE_TEXT[co.lp_stage] ?? "text-slate-600")}>{co.lp_stage}</span>
                        </div>
                      ) : <span className="text-slate-300 text-xs">—</span>}
                    </td>
                    <td className="px-3 py-2.5 text-right text-xs text-slate-700 tabular-nums whitespace-nowrap">{formatCompact(co.commitment_goal)}</td>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={cn("text-xs tabular-nums", p > 0 ? "text-emerald-600 font-medium" : "text-slate-400")}>{formatPct(p)}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                      {touch ? (
                    ...
                    <td className="px-3 py-2.5 text-xs text-slate-500 whitespace-nowrap">
                      {co.last_contact_date ? formatDate(co.last_contact_date) : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-3 py-2.5">
                      <span className={cn("text-[11px] px-2 py-0.5 rounded-full font-medium whitespace-nowrap", ddq.color)}>{ddq.label}</span>
                    </td>
                    <td className="px-3 py-2.5">
          ...
          {selected && (
            <>
              ...
              <div className="flex gap-2 px-5 py-3 border-b border-slate-100">
                ...
                {/* Update Stage dropdown */}
                <div className="relative flex-1" ref={updateStageRef}>
                  <button
                    onClick={() => setShowUpdateStage((v) => !v)}
                    className="w-full flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 justify-center"
                  >
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg z-10 overflow-hidden">
                      {LP_STAGE_OPTIONS.map((s) => (
                        <button
                          key={s}
                          onClick={async () => {
                            await saveField(selected.id, { lp_stage: s });
                            setShowUpdateStage(false);
                          }}
                          className={cn(
                            "w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-slate-50 text-left",
                            editStage === s ? "bg-blue-50" : ""
                          )}
                        >
                          <span className={cn("w-2 h-2 rounded-full flex-shrink-0", STAGE_DOT[s])} />
                          <span className={STAGE_TEXT[s]}>{s}</span>
                          {editStage === s && <Check size={11} className="ml-auto text-blue-600" />}
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={selected.lp_type ?? ""}
                        onChange={async (e) => {
                          await saveField(selected.id, { lp_type: e.target.value || null });
                        }}
                      >
                    ...
                    <div>
                      ...
                      >
                        <option value="">Not set</option>
                        {LP_STAGE_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </div>
                    {/* Commitment Goal — editable */}
                    <div>
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-1">Commitment Goal</p>
                      <input
                        type="number" placeholder="e.g. 5000000" value={editGoal}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 py-1.5">
                        ...
                        <span className={cn("text-xs font-bold tabular-nums", prob > 0 ? "text-emerald-600" : "text-slate-400")}>
                          {formatPct(prob)}
                        </span>
                      </div>
                    ...
                    <div>
                      ...
                      <span className="text-xs font-medium text-slate-700 py-1 inline-block">{getTier(selected.priority)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() => { setContactsManaging((v) => !v); setShowAddContactForm(false); }}
                        ...
                        <div key={c.id} className="flex items-start gap-2.5 p-3 border border-slate-100 rounded-xl hover:border-blue-200 hover:bg-blue-50 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {(c.location_city || c.location_country) && (
                              <p className="text-[11px] text-slate-400">{[c.location_city, c.location_country].filter(Boolean).join(", ")}</p>
                            )}
                          </div>
                          <button
                            onClick={async () => {
                            ...
                            className="w-6 h-6 flex items-center justify-center rounded-lg border border-slate-200 text-slate-400 hover:border-red-200 hover:text-red-400 flex-shrink-0"
                          >
                            <X size={11} />
                          </button>
                        </div>
                        ...
                        <div className="border border-blue-200 rounded-xl bg-blue-50 p-3 space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <input className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                              placeholder="First name" value={newContact.first_name}
                              onChange={(e) => setNewContact((p) => ({ ...p, first_name: e.target.value }))} />
                            <input className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                              placeholder="Last name" value={newContact.last_name}
                          ...
                          <input className="w-full text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                            placeholder="Title / Role" value={newContact.title}
                            onChange={(e) => setNewContact((p) => ({ ...p, title: e.target.value }))} />
                          <div className="grid grid-cols-2 gap-2">
                            <input className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                              placeholder="City" value={newContact.location_city}
                              onChange={(e) => setNewContact((p) => ({ ...p, location_city: e.target.value }))} />
                            <input className="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                              placeholder="Country" value={newContact.location_country}
                              onChange={(e) => setNewContact((p) => ({ ...p, location_country: e.target.value }))} />
                          </div>
                          <div className="flex gap-2">
                            <button onClick={() => { setShowAddContactForm(false); setNewContact({ first_name: "", last_name: "", email: "", title: "", location_city: "", location_country: "" }); }}
                              className="flex-1 py-1.5 border border-slate-200 rounded-lg text-xs text-slate-600 hover:bg-white">Cancel</button>
                    ...
                    <div className="mb-3 p-3 border border-blue-200 rounded-xl bg-blue-50 space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input type="date"
                            className="w-full text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                        ...
                        <div>
                          ...
                          <select
                            className="w-full text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                            value={activityType} onChange={(e) => setActivityType(e.target.value as "call" | "meeting" | "email")}
                          >
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button onClick={() => { setAddingActivity(false); setActivityNote(""); }}
                          className="text-xs px-3 py-1.5 border border-slate-200 rounded-lg text-slate-500 hover:bg-white">Cancel</button>
      ...
      {showPrepBrief && selected && (
        <PrepBriefModal
          ...
          onClose={() => setShowPrepBrief(false)}
        />
      )}
    </div>
  );
