        ...
        <div className="card p-4 md:p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              {/* Creator / regenerator badge */}
              <div className="flex items-center gap-2 text-xs text-slate-500 mt-1 flex-wrap">
                {(memo as any).created_by_profile && (
                  <span className="inline-flex items-center gap-1.5">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-700" title={`Created by ${(memo as any).created_by_profile.full_name}`}>
                      {(memo as any).created_by_profile.initials ?? "?"}
                    </span>
                    <span>Created by {(memo as any).created_by_profile.full_name}</span>
                  </span>
                )}
                {(memo as any).regenerated_by_profile && (
                  <span className="inline-flex items-center gap-1.5 ml-2">
                    <span className="inline-flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-semibold bg-violet-100 text-violet-700" title={`Regenerated by ${(memo as any).regenerated_by_profile.full_name}`}>
                      {(memo as any).regenerated_by_profile.initials ?? "?"}
                    </span>
                    <span>Regenerated {(memo as any).regenerated_at ? new Date((memo as any).regenerated_at).toLocaleDateString() : ""}</span>
                  </span>
                )}
              </div>
            </div>
